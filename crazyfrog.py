...
def buzz(frequency, length):	 			#create the function "buzz" and feed it the pitch and duration)
	...
	for i in range(numCycles):				#start a loop from 0 to the variable "cycles" calculated above
		GPIO.output(buzzer_pin, True)	 	#set pin 23 to high
		time.sleep(delayValue)				#wait with pin 23 high
		GPIO.output(buzzer_pin, False)		#set pin 23 to low
		time.sleep(delayValue)				#wait with pin 23 low
